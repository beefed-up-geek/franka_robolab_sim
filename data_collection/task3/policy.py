...
#
# 손끝이 0.325 까지 올라간다. **기준은 손끝이 아니라 든 캔의 바닥**이다 —
# 캔은 허리를 물려 손끝 아래로 반높이만큼 내려와 있다. 0.03(손끝 0.305)일 때
# 든 캔 바닥이 0.284 라 이웃 캔 꼭대기(정상 0.258, 파열 0.283)에 1mm 차이로
# 스쳤고, 실제로 운반 중 옆 캔을 쳐 넘어뜨렸다. 0.05 면 바닥이 0.304 로
# 파열 캔 위로도 21mm 남는다.
LIFT_H = 0.05
...
class PickPlacePolicy:
    ...
    def _goal(self, can, eef, flange_offset):
        ...
        if self.stage in ("TO_BIN", "OPEN"):
            goal = [BIN_XY[0], BIN_XY[1], BIN_DROP_TCP_Z + flange_offset]
            # 통 높이는 운반 고도보다 **낮다**. 곧장 통 좌표로 가면 대각선 경로가
            # 벨트 상공에서부터 하강해, 든 캔 바닥이 이웃 캔 꼭대기를 스친다
            # (실측: 운반 중 옆 캔을 쳐 넘어뜨림 — 두 번째 캔을 집을 때는 첫
            # 캔 바로 위를 지나므로 특히 잦다). 벨트 영역(x>0.38)을 벗어나기
            # 전에는 LIFT 가 잠근 운반 고도를 유지하고, 벗어난 뒤 내려간다.
