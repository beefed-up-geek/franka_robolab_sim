...
#
# 1.4 에서 1.8 로 올렸다. 접근 경로에 무작위 경유점(policy.py VIA)이 들어가면서
# 길이 편차가 커졌는데, 기준이 빡빡하면 **가장 크게 돌아가는 궤적만 골라서**
# 버리게 된다 — 다양성을 넣어 놓고 그 다양성을 도로 걷어내는 꼴이다.
# 머뭇거림(정지 구간)을 거르는 것이 원래 목적이고, 그건 이 배율을 넘길 만큼
# 심할 때만 문제가 된다.
OUTLIER_FACTOR = 1.8
